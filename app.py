...
from utils.cache import limpiar
...
def sobregiros():
    ...
    return render_template("sobregiros.html", s=s, lista=clientes)
...
def cliente(interlocutor):
    ...
    return render_template("sobregiros_cliente.html", s=cliente_s)
...
@app.route("/api/cliente/<interlocutor>/exportar")
def exportar_sobregiro_cliente(interlocutor):

    cliente_s = sobregiro.sobregiro_cliente(interlocutor)
    df = cliente_s["historial"]
    ...
    return send_file(
        ...
        mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        as_attachment=True,
        download_name=f"sobregiro_{interlocutor}.xlsx",
    )
...
def incumplimientos():
    ...
    return render_template("incumplimientos.html", s=s, lista=clientes)
...
def cliente_incumplimiento(interlocutor):
    ...
    return render_template("incumplimientos_cliente.html", s=cliente_i)
...
@app.route("/api/incumplimientos/cliente/<interlocutor>/exportar")
def exportar_incumplimiento_cliente(interlocutor):

    cliente_i = incumplimiento.incumplimiento_cliente(interlocutor)
    df = cliente_i["historial"]
    ...
    return send_file(
        ...
        mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        as_attachment=True,
        download_name=f"incumplimiento_{interlocutor}.xlsx",
    )


@app.route("/admin/cache/limpiar", methods=["GET", "POST"])
def limpiar_cache():

    limpiar()

    return jsonify({"ok": True, "mensaje": "Caché vaciado"})


if __name__ == "__main__":
    from waitress import serve

    serve(app, host="0.0.0.0", port=2001, threads=8)