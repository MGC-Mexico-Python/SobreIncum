...
class Sobregiros:
    def __init__(self, conexion: PostgreSQL):
    ...
    def _sin_sobregiro_cliente(self, cliente):
    ...
    def _analisis_cliente(self, query: pd.DataFrame, cliente) -> dict:

        fila_hoy = query[query["Fecha"] == query["Fecha"].max()].iloc[0]

        monto_hoy = money(
            fila_hoy["Monto sobregiro"]
        ...
        query["Condiciones de pago"] = pd.to_numeric(
            query["Condiciones de pago"].str[2:]
        )
        query["Dias sin sobregiro"] = Calendario.diferencia_habil(query["Fecha"])

        query["Ratio recurrencia"] = round(
            query["Dias sin sobregiro"] / query["Condiciones de pago"], 2
        ).clip(upper=1)
        query["Ratio monto"] = round(
            query["Monto sobregiro"] / query["Límite de credito"], 2
        )
    ...
    def _score_sobregiro(self, datos: dict) -> dict:
        ...
        return {"clasificación": clasificacion, "mensaje": mensaje.strip()}

    @cacheado
    def sobregiro_cliente(self, cliente):
        ...
        query_semanal_monto = (
            query_semanal.groupby(["Fecha"])["Monto sobregiro"]
            .mean()
            .reindex(dias, fill_value=0)
            .reset_index()
        )

        query_semanal_eventos = (
            query_semanal.groupby(["Fecha"])["Interlocutor"]
            .count()
            .reindex(dias, fill_value=0)
            .reset_index()
        )
        ...
        query_mensual_monto = (
            query_mensual.groupby(["Fecha"])["Monto sobregiro"]
            .mean()
            .reindex(meses, fill_value=0)
            .reset_index()
        )

        query_mensual_eventos = (
            query_mensual.groupby(["Fecha"])["Interlocutor"]
            .count()
            .reindex(meses, fill_value=0)
            .reset_index()
        )
